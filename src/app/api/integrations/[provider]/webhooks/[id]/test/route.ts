...
export async function POST(_req: Request, { params }: { params: { provider: string; id: string } }) {
  try {
    const wh = webhooksDao.findById(params.id);
    if (!wh) throw new NotFoundError(`Webhook ${params.id} not found`);
